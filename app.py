...
def predict_frames():
    ...
    uploaded_files = request.files.getlist('files')
    save_folder = "/output"  # Update this with the desired folder path

    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    results = []  # To store the prediction results
    ...
    for file in uploaded_files:
        ...
        # Find the index of the most predicted class
        index_of_max_value = np.argmax(predic[0])
        # Add the prediction result to the list
        results.append(index_of_max_value)

    # Find the index with the highest number of predictions
    counter = Counter(results)
    mc = counter.most_common(1)[0][0]
    return jsonify({"result": classes3[mc]})
